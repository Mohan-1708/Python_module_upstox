...
# # import config
# # from src.utils import setup_logging, load_stocks_list, save_results_csv
...
# # def main():
# #     """
# #     Main function to run the data fetching, strategy, and backtesting pipeline.
# #     """
# #     setup_logging()
# #     logging.info(f"Starting backtest for strategy: {config.STRATEGY_NAME}")
# #
# #     # 1. Load Stocks
...
# #
# #     # 3. Fetch Data and Generate Signals for each stock
# #     logging.info(f"Fetching data and generating signals for {len(stocks_df)} stocks...")
# #     for index, row in stocks_df.iterrows():
# #         stock_symbol = row['Symbol']
# #         instrument_key = f"NSE_EQ|{row['ISIN Code']}"
# #
# #         logging.info(f"Processing: {stock_symbol} ({instrument_key})")
# #
# #         try:
# #             # Fetch data
...
# #
# #             if df_5m.empty:
# #                 logging.warning(f"No data fetched for {stock_symbol}. Skipping.")
# #                 continue
# #
# #             all_stocks_data[stock_symbol] = df_5m
# #
# #             # Generate signals
...
# #
# #         except Exception as e:
# #             logging.error(f"Error processing {stock_symbol}: {e}", exc_info=True)
# #
# #     # 4. Run Backtest
# #     logging.info(f"Total signals generated: {len(all_combined_signals)}")
# #     if not all_combined_signals:
# #         logging.info("No signals found. Exiting.")
# #         return
# #
# #     all_combined_signals_df = pd.DataFrame(all_combined_signals)
# #
# #     logging.info("Running combined backtest...")
# #     backtest_results_df = backtest_strategy_combined(all_combined_signals_df, all_stocks_data)
# #
...
# #
# #         save_results_csv(backtest_results_df, config.OUTPUT_CSV_FILENAME)
# #     else:
...
# import config
# # UPDATE this import
# from src.utils import setup_logging, load_stocks_list, save_results_to_db
# from src.data_fetcher import get_api_client, get_continuous_candles
# from src.strategy import generate_signals
# from src.backtester import backtest_strategy_combined
#
...
#     logging.info(f"Starting backtest for strategy: {config.STRATEGY_NAME}")
#
#     # ... (Steps 1, 2, 3, and 4 remain exactly the same) ...
#
#     # 1. Load Stocks
#     logging.info(f"Loading stock list from: {config.STOCKS_CSV_PATH}")
#     stocks_df = load_stocks_list(config.STOCKS_CSV_PATH)
...
#     backtest_results_df = backtest_strategy_combined(all_combined_signals_df, all_stocks_data)
#
#     # 5. Summarize and Save Results (UPDATED)
#     if not backtest_results_df.empty:
#         overall_profit_loss = backtest_results_df['Profit_Loss'].sum()
#         total_trades = len(backtest_results_df)
#         wins = len(backtest_results_df[backtest_results_df['Outcome'] == 'Win'])
...
#         logging.info(f"Overall Profit/Loss: {overall_profit_loss:.2f}")
#
#         # Call the new database save function
#         save_results_to_db(
